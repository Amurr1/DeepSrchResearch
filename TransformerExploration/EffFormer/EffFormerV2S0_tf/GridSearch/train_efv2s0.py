...
def preprocess_dataset(ds="deluxe", bs=1024):
    xtrain, ytrain, xval, yval, input_shape = None
    if ds == "deluxe":
...
def create_eff_former(lr, input_shape):
    base_model = keras_cv_attention_models.efficientformer.EfficientFormerV2S0(input_shape=input_shape, pretrained='imagenet')
...
def train_model(train, validate, input_shape, steps_per_epoch, validation_steps, lr, bs, log_csv, w_chkpt, w_end):
    model = create_eff_former()
...
def main():
    ...
    train_model(train, validate, input_shape, steps_per_epoch, validation_steps, learning_rate, batch_size, log_csv, w_chkpt, w_end)
